(function () {
  ...
  function summarizeProviderConfig(value) {
    const fetchedModels = Array.isArray(value?.fetchedModels)
      ...
      : [];
    const hasBaseUrl =
      typeof value?.hasBaseUrl === "boolean"
        ? value.hasBaseUrl
        : !!value?.baseUrl;
    const hasApiKey =
      typeof value?.hasApiKey === "boolean" ? value.hasApiKey : !!value?.apiKey;
    const hasDefaultModel =
      typeof value?.hasDefaultModel === "boolean"
        ? value.hasDefaultModel
        : !!value?.defaultModel;
    return {
      ...
      reasoningEffort: sanitizeString(
      ...
      fetchedModelCount:
        typeof value?.fetchedModelCount === "number"
          ? value.fetchedModelCount
  ...
  function safeClone(value, depth, seen, parentKey) {
    ...
    if (typeof Event !== "undefined" && value instanceof Event) {
      return {
        type: value.type,
        target:
          value.target && value.target.tagName
    ...
    if (
      normalizeKey(parentKey) === "customproviderconfig" ||
      normalizeKey(parentKey) === "customprovider"
    ) {
    ...
    for (const key of Object.keys(value).slice(0, 30)) {
      output[key] = isSensitiveKey(key)
        ? REDACTED_SECRET
        : safeClone(value[key], depth + 1, seen, key);
    }
    return output;
  }
  ...
  async function read() {
    ...
    return Array.isArray(stored[STORAGE_KEY])
      ? sanitizeLogEntries(stored[STORAGE_KEY])
      : [];
  }
